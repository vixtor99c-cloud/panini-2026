...
// ── OCR: pre-procesar un canvas para máxima legibilidad ──────────────────────
// Las cartas Panini 2026 tienen el badge de siglas en la esquina superior derecha:
// fondo gris oscuro, texto BLANCO en negrita → necesitamos invertir para OCR.
function preprocessCanvas(src, invert = false) {
  // Escalar a resolución óptima (mín 1200px en el lado más largo)
  const sc  = Math.max(1, Math.min(5, 1400 / Math.max(src.width, src.height, 1)));
  const dst = document.createElement("canvas");
  dst.width  = Math.round(src.width  * sc);
  dst.height = Math.round(src.height * sc);
  ...
  for (let i = 0; i < d.length; i += 4) {
    ...
    if (invert) g = 255 - g;         // invertir para texto blanco sobre oscuro
    // Umbral adaptativo: realza el contraste
    const v = g > 118 ? 255 : 0;
    d[i] = d[i+1] = d[i+2] = v;
    d[i+3] = 255;
  }
...
function cropBadgeZones(src) {
  ...
  // 1) Imagen completa (siempre)
  zones.push({ canvas: src, label: "full" });

  // 2) Dividir en N franjas verticales para cubrir cartas que se solapan.
  //    Con 2-5 cartas abanicadas, cada una ocupa ~15-25% del ancho visible.
  const slices = [1, 2, 3, 4]; // cuántos cortes hacer en el eje X
  for (const n of slices) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      c.width  = sliceW;
      c.height = H;
      c.getContext("2d").drawImage(src, x, 0, sliceW, H, 0, 0, sliceW, H);
      zones.push({ canvas: c, label: `slice_${n}_${i}` });
    }
  }

  // 3) Recorte enfocado: esquina superior de cada franja (top 25%) — donde está el badge
  for (let i = 0; i < 4; i++) {
    const x = i * Math.floor(W / 4);
    const w = Math.floor(W / 4);
    const h = Math.floor(H * 0.28); // top 28% de la carta
    const c = document.createElement("canvas");
    c.width = w; c.height = h;
    c.getContext("2d").drawImage(src, x, 0, w, h, 0, 0, w, h);
    zones.push({ canvas: c, label: `top_${i}` });
  }
...
// ── Busca la sigla FIFA más cercana al texto OCR ──────────────────────────────
function matchFifaCode(raw) {
  if (!raw || raw.length < 2) return null;
  if (FIFA_CODES.has(raw)) return raw;
  // Confusiones comunes del OCR en texto blanco sobre gris oscuro
  const fixes = { "0":"O","1":"I","5":"S","8":"B","6":"G","2":"Z" };
  const cleaned = raw.split("").map(c=>fixes[c]||c).join("");
  if (FIFA_CODES.has(cleaned)) return cleaned;
  // Fuzzy: distancia ≤ 1 entre siglas del mismo largo
  let best = null, bestDist = 2;
  for (const fc of FIFA_CODES) {
    if (Math.abs(fc.length - raw.length) > 1) continue;
    const d = levenshtein(fc, raw.length === fc.length ? raw : raw.substring(0, fc.length));
    if (d < bestDist) { bestDist = d; best = fc; }
  }
  return best;
}

// ── OCR: extraer pares "SCO 16", "CPV 1", "ARG 15" del texto crudo ───────────
function extractCardsFromText(raw, seen = new Set()) {
  const results = [];
  // Normalizar: mayúsculas, colapsar espacios/saltos
  const text = raw.toUpperCase()
    .replace(/[^A-Z0-9\s]/g, " ")   // quitar caracteres raros
    .replace(/\s+/g, " ")
    .trim();

  // Patrón: 2-4 letras + separador opcional + 1-2 dígitos
  // Cubre "SCO16", "SCO 16", "SCO  16", "SCO\n16"
  const re = /\b([A-Z]{2,4})\s{0,4}(\d{1,2})\b/g;
  let m;
  while ((m = re.exec(text)) !== null) {
    ...
    // Filtrar palabras comunes del texto de la carta que no son siglas
    const IGNORE = new Set(["FIFA","CUP","THE","AND","ALL","ARE","FOR","ITS",
      "WWW","COM","MADE","UNDER","BY","IN","SP","LTD","SPA","BRASIL","BARUERI",
      "OFFICIAL","LICENSED","PRODUCT","PANINI","LOGOS","BRAND","TRADE","NAMES",
      "EVENTS","PRODUCTS","TRADEMARKS","ESTE","PARA","DOS","LOS","DEL"]);
    if (IGNORE.has(rawCode)) continue;
    if (rawCode.length === 1) continue;

    const code = matchFifaCode(rawCode);
    if (!code) continue;
